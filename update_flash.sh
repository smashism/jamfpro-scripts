...
###
#
#            Name:  update_flash.sh
#     Description:  This script downloads and installs the latest Flash player for compatible Macs.
#          Author:  Rich Trouton (github.com/rtrouton)
#            Note:  Modifications include updated formatting to remove deprecated styling, and
#                   additional echoes for more thorough logging when this runs through the JSS.
#     Modified By:  Emily Kausalik (drkausalik@gmail.com)
#         Created:  2017-01-11
...
echo "Current Adobe Flash Player is $flash_version."
...
if [[ ${osvers} -ge 6 ]]; then
    ...
    if [[ ${pkg_path} != "" ]]; then
       if [[ ${osvers} -ge 7 ]]; then
         ...
         if [[ ${signature_check} = "Adobe" ]]; then
           ...
           echo "Installing Adobe Flash Player $flash_version..."
    ...
    echo "Adobe Flash Player $flash_version installed. Cleaning up..."
